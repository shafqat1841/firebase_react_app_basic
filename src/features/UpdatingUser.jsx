...
// UpdatingUser conponent starts here
// UpdatingUser component sets user id as a value in the state of UserProvider component (which gives UserContext context api
// values to provide in child component)
// UpdatingUser component will have one prop docRefId (which is an id from firestore of a particular document)
// ---------------------------------------------- component Starts ---------------------------------------------------
const UpdatingUser = ({ docRefId }) => {
	// taking setUpdatingUserDocRef state setter function from context api by using UserUseContext
	// setUpdatingUserDocRef state setter function sets user id as a value in updatingUserDocRef state in UserProvider component
	...
	// In UpdatingUser component we are returning a ButtonCom component
	return (
		<ButtonCom
			...
			onClick={useCallback(() => {
				// when the button is click then we sent docRefId (user id) in setUpdatingUserDocRef state setter function
				// it sets user id as a value in updatingUserDocRef state in UserProvider component
				setUpdatingUserDocRef(docRefId);
				// we have pass next page address ("/UpdatingUser") in navigate to get to that page
				// when the button is click it navigate us to UpdateUser page (UpdatePage component) which we have defined
...
// UpdatingUser is exported and is used in ShowingAllUsers compnent as a button
export default memo(UpdatingUser);
